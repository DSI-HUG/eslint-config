...
import { dedupeByName, isPackageInstalled } from './common/utils';
...
const createRecommended = (options?: CreateOptions | false): Config[] => {
    if (options === false) {
        return defineConfig({
            name: '@hug-eslint-config/typescript (disabled)',
        });
    } else if (!isPackageInstalled('typescript')) {
        return defineConfig({
            name: '@hug-eslint-config/typescript (not applicable)',
        });
    } else {
        return defineConfig(
            {
                name: '@hug-eslint-config/typescript/base',
                files: options?.files ?? Files.TYPESCRIPT,
                languageOptions: {
                    parserOptions: {
                        projectService: true,
            ...
            dedupeByName([
                ...
                c.files = options?.files ?? Files.TYPESCRIPT;
                return c;
            }),

            {
                name: '@hug-eslint-config/typescript/recommended',
                files: options?.files ?? Files.TYPESCRIPT,
                rules: {
